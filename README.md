[README.md](https://github.com/user-attachments/files/28083161/README.md)
# NOVA TECH CAREERS — 採用 LP

> **テクノロジーで、次の時代をつくる。**
> 近未来系 IT 企業を想定した、架空企業の採用ランディングページ。

採用 LP として、企画・情報設計・実装・公開までを一人で担当したポートフォリオ作品です。「ただ綺麗なサイト」ではなく、応募数・応募者の質・エントリー完了率を改善できる構造を意識して制作しました。
...
近未来的な世界観の中で、求職者が「何の会社か」「自分が働けるか」「次に何をすればいいか」を迷わず理解し、エントリーまで到達できる構造を 1 枚に凝縮することを意識しました。
...
採用 LP の離脱は、ファーストビューの数秒以内に集中して発生しやすいとされています。
本サイトは「見栄えの良さ」だけを目的とせず、以下の指標を改善できる LP として設計しました。

- ファーストビューでの価値訴求 → 不安解消 → エントリーまでの動線を 1 枚で完結させる
- 「合うか／働けるか」という求職者の不安を、文化・制度・社員の声で段階的に解消する
- 移動中のスマートフォン操作でもストレスなくエントリーできる状態をつくる

架空企業の案件ですが、「実在のクライアントから依頼されたつもりで考える」を前提に、ヒアリング項目・ペルソナ・KPI 仮説・サイトマップを言語化してからコードに進む手順を取っています。
...
**Apple × SaaS × Glass UI** — 情報密度を保ったまま、静かに先進性を伝えるビジュアル言語をめざしました。

| 領域 | 方針 |
| --- | --- |
| Color | 背景 `#0a0b14` の漆黒をベースに、青紫グラデーション（`#5b8def → #8b5cf6 → #c084fc`）をアクセントに限定使用。黒の比重を高く保つ |
...
| Motion | ヒーローのオーブを長周期で漂わせ、セクションはフェード＋わずかな Y 軸移動のみ。動きすぎず、無反応でもない手応えを意識 |

黒は単なる高級感の演出ではなく、グラデーション・ガラス効果・数値を最大コントラストで際立たせる「土台」として機能させています。
...
| # | 項目 | 内容 |
| --- | --- | --- |
| 01 | FLOCSS / BEM 風命名 | `l-` `c-` `p-` `u-` プレフィックスで役割を分離し、複数人が触っても破綻しにくい設計 |
| 02 | CSS 変数によるトークン管理 | 配色・余白・タイポを `:root` に集約。デザイン調整を 1 箇所で完結できる構造 |
| 03 | IntersectionObserver | スクロール連動のフェードインを実装。一度可視化したら監視解除し CPU 負荷を抑制 |
| 04 | ハンバーガーメニュー | 開閉／ESC キー／リンク選択／リサイズで閉じる動作と `aria-expanded` を同期 |
| 05 | Glassmorphism | `backdrop-filter` を限定使用。ドロワー背景は不透明にして可読性を確保 |
| 06 | SEO / OGP / JSON-LD | `title` / `description` 最適化、OGP 画像、Organization Schema を JSON-LD で配置 |
| 07 | アクセシビリティ | スキップリンク・`aria-label`・フォーカスリングを丁寧に。色のコントラスト比 WCAG AA を基準 |
...
「将来 WordPress 化する前提」と「保守できる範囲に収める」のバランスを取った技術選定です。
...
- DevTools のデバイスモードで主要な端末幅（375 / 390 / 430 / 768 / 1024 / 1280px）を確認し、横スクロール・レイアウトずれが出ないよう調整
...
| OGP | `preview-mockup.webp` を OGP 画像として設定し、SNS シェア時の表示を最適化 |
| 構造化データ | Organization Schema を JSON-LD で記述 |
| アクセシビリティ | スキップリンク、`aria-*` 属性、フォーカスリング、WCAG AA を基準にしたコントラスト |
...
└── assets/
    └── images/
        ├── preview-mockup.webp  # READMEヒーロー & OGP / SNSシェア用ビジュアル
        └── qr.png               # スマホアクセス用 QR
```
